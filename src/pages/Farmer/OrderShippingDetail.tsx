...
const FarmerOrderShippingDetail: React.FC<Props> = ({ sellerId }) => {
    ...
    const fetchOrdersAndShipping = async () => {
        setLoading(true);
        try {
            ...
            const results = await Promise.all(
                list.map(async (o) => {
                    ...
                    if (oid === undefined) {
                        console.warn('Thiếu order_id/id trong order:', o);
                        return [null, null] as [number | null, ShippingInfo | null];
                    }
                    try {
                        const shipRes = await fetch(`${API_BASE}/shipping/${oid}`);
                        if (shipRes.status === 404) return [oid, null];
                        const shipData = await shipRes.json().catch(() => null);
                        return [oid, shipRes.ok ? (shipData.shipping || shipData) : null];
                    } catch {
                        return [oid, null];
                    }
                })
            ...
            setError('Không thể tải đơn hàng hoặc vận chuyển');
        }
        setLoading(false);
    };
    ...
    async function updateProductStatus(orderId: number, productId: number, newStatus: string) {
        try {
            const res = await fetch(`${API_BASE}/orders/${orderId}/product/${productId}/status`, {
                method: 'PUT',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({ 
                    status: newStatus,
                    seller_id: sellerId  // ✅ THÊM: Gửi kèm seller_id để backend kiểm tra quyền
                })
            });
            
            if (!res.ok) {
                const err = await res.json().catch(() => ({ error: 'Unknown error' }));
                console.warn('⚠️ Update product status failed:', err);
                return false;
            }
            
            const data = await res.json();
            console.log('✅ Product status updated:', data);
            return data; // Trả về { order_status, message }
        } catch (e: any) {
    ...
    async function handleShippingStatusChange(orderIdRaw: number, newStatus: string) {
        ...
        try {
            ...
            // ✅ TỰ ĐỘNG TÍNH shipped_at và delivered_at
            ...
            // 2. CẬP NHẬT TRẠNG THÁI SẢN PHẨM (API mới)
            console.log('📤 Updating product status via new API...');
            const productUpdateResult = await updateProductStatus(orderId, productId, newStatus);
            
            if (productUpdateResult) {
                // ✅ RELOAD DỮ LIỆU TỪ BACKEND
                await fetchOrdersAndShipping();
                
                // ✅ BACKEND TỰ ĐỘNG TÍNH order_status → Không cần gọi updateOrderStatus nữa
                const updatedOrder = orders.find(o => getOrderId(o) === orderId);
                const finalStatus = updatedOrder?.status || newStatus;
                
                setNotify(`✅ Đã cập nhật!`);
            } else {
                await fetchOrdersAndShipping();
                setNotify(`⚠️ Đã cập nhật vận chuyển`);
            }
        } catch (err: any) {
    ...
    function openCreateForm(orderIdRaw: number) {
        const orderId = orderIdRaw;
        setShowCreateForm(f => ({ ...f, [orderId]: true }));
        setCreateForm(f => ({
            ...f,
            [orderId]: {
                shipping_company: shippingCompanies[0],
    ...
    return (
        <div style={{ padding: '8px 12px' }}>
            ...
            {notify && (
                <div style={{
                    background: notify.includes('thành công') ? '#38b000' : '#D32F2F',
                    color: '#fff',
                    padding: '10px 16px',
                    borderRadius: 8,
                    marginBottom: 16,
                    ...
                    <span>{notify.includes('thành công') ? '✔' : '✖'}</span>
                    <span>{notify}</span>
                </div>
            )}
                                    ...
                                    >
                                        ...
                                        <div style={{ display: 'flex', gap: 10, justifyContent: 'center' }}>
                                            ...
                                            <button
                                                ...
                                                onClick={() => setShowCreateForm((f) => ({ ...f, [oid]: false }))}
                                            >
                                                Hủy
                                            </button>
                                        </div>
